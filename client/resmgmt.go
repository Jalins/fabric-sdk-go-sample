...
func (c *Client) InstallCC(v string, peer string) error {
	...
	for _, resp := range resps {
		...
		if resp.Info == "already installed" {
			...
			return errors.WithMessage(errs[0],resp.Info)
		}
	}
...
func (c *Client) InvokeCC(peers []string) (channel.Response, error) {
	...
	if err != nil {
		return resp, errors.WithMessage(err, "invoke chaincode error")
	}

	return resp, nil
}

func (c *Client) InvokeCCDelete(peers []string, key string) (channel.Response, error) {
...
func (c *Client) QueryCC(peer, keys string) (channel.Response, error) {
	...
	if err != nil {
		return resp, errors.WithMessage(err, "query chaincode error")
	}
